    ...
    public function enviar($id_processo,$estado,$id_tecnico=null)
    {
        ...
        switch ($estado)
        {
            case 1:
                ...
                break;
            case 2:
                if($this->parecer->control_envio($this->session->userdata('id'),$id_processo)>0)
                {
                    if(strstr($this->parecer->get_parecer($id_processo)->descricao,"reprovado"))
                    {
                        ...
                        $id_entrada = $ret->id;
                        $entrada_saida = new \Entity\Entrada_saida($this->session->userdata('id'), $id_entrada, $id_processo, 3);//depois vamos discutir
                        $e_s = $this->entrada_saida->store($entrada_saida);
                        if($this->alterar_estado_processo(0,$id_processo) && $e_s!=0) {
                            $e_s = 1;
                        }else {
                            $e_s = 0;
                        }
...
?>
<script src=<?= site_url("public/assets/pages/scripts/charts-flotcharts.min.js")?> type="text/javascript"></script>

<script src=<?= site_url("public/assets/global/plugins/icheck/icheck.min.js");?> type="text/javascript"></script>
<script src=<?= site_url("public/assets/global/plugins/flot/jquery.flot.min.js")?> type="text/javascript"></script>
<script src=<?= site_url("public/assets/global/plugins/flot/jquery.flot.resize.min.js")?> type="text/javascript"></script>
<script src=<?= site_url("public/assets/global/plugins/flot/jquery.flot.categories.min.js")?> type="text/javascript"></script>
<script src=<?= site_url("public/assets/global/plugins/flot/jquery.flot.pie.min.js")?> type="text/javascript"></script>
<script src=<?= site_url("public/assets/global/plugins/flot/jquery.flot.stack.min.js")?> type="text/javascript"></script>
<script src=<?= site_url("public/assets/global/plugins/flot/jquery.flot.crosshair.min.js")?> type="text/javascript"></script>
<script src=<?= site_url("public/assets/global/plugins/flot/jquery.flot.axislabels.js")?> type="text/javascript"></script>
